<%@ Page Title="Seven Day Home Solar PV and Water Current Report" Language="C#" MasterPageFile="~/archive/MasterPage.master" %>
...
<script runat="server">
    ...
    public void setdata(DateTime dt){
        ...
        SqlDataSource1.SelectCommand = "SELECT TOP (100) PERCENT avgwatertop, avgwaterbase, avgwaterpanel, avgwhometemp, EntryHour, DATEPART(dd, EntryDate) AS DateDay FROM dbo.FullArchive WHERE EntryDate > '" + dt.AddDays(-7).ToString("yyyy-MM-dd") + "' Order BY EntryDate ASC";
        Repeater1.DataBind();
            
        System.Data.DataSet dsTax = new System.Data.DataSet();
        System.Data.SqlClient.SqlConnection objConn = new System.Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings["MainConn"].ConnectionString);
        objConn.Open();
        System.Data.SqlClient.SqlDataAdapter daTax = new System.Data.SqlClient.SqlDataAdapter("SELECT MAX(maxwaterbase) as maxwaterbase, MAX(maxbatteryv) AS maxbatteryv,MAX(maxwatertop) AS maxwatertop, MAX(maxwaterpanel) AS maxwaterpanel, MAX(maxhometemp) AS maxhometemp, MAX(maxmainsc) AS maxmainsc, MAX(maxsolarc) AS maxsolarc, MAX(maxgeneralc) AS maxgeneralc  FROM dbo.FullArchive  WHERE EntryDate > '" + dt.AddDays(-7).ToString("yyyy-MM-dd") + "' AND EntryDate < '" + dtEnd.ToString("yyyy-MM-dd") + "'", objConn);
        ...
        SqlDataSourcePV.SelectCommand = "SELECT avgmainsc, avgsolarc, avggeneralc , EntryHour , DATEPART(dd, EntryDate) AS DateDay  FROM dbo.FullArchive WHERE EntryDate > '" + dt.AddDays(-7).ToString("yyyy-MM-dd") + "' Order BY EntryDate ASC";
        Repeater2.DataBind();

        // battery data
        SqlDataSourceBattery.SelectCommand = "SELECT  avgbatteryv , EntryHour, DATEPART(dd, EntryDate) AS DateDay   FROM dbo.FullArchive WHERE EntryDate > '" + dt.AddDays(-7).ToString("yyyy-MM-dd") + "' Order BY EntryDate ASC";
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
 ...
 <script type="text/javascript" src="https://www.google.com/jsapi"></script>
  ...
  <ItemTemplate>
      ...
      ['<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateDay").ToString()) %>:<%# CheckLen(DataBinder.Eval(Container.DataItem, "EntryHour").ToString()) %>', <%# DataBinder.Eval(Container.DataItem, "avgwatertop") %>, <%# DataBinder.Eval(Container.DataItem, "avgwaterpanel") %>, <%# DataBinder.Eval(Container.DataItem, "avgwhometemp") %>],
  ...
  <ItemTemplate>
      ...
      ['<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateDay").ToString()) %>:<%# CheckLen(DataBinder.Eval(Container.DataItem, "EntryHour").ToString()) %>', <%# DataBinder.Eval(Container.DataItem, "avgsolarc") %>, <%# DataBinder.Eval(Container.DataItem, "avggeneralc") %>, <%# DataBinder.Eval(Container.DataItem, "avgmainsc") %>],
  </ItemTemplate>
</asp:Repeater>
            ]);
  ...
  <ItemTemplate>
      ...
      ['<%# CheckLen(DataBinder.Eval(Container.DataItem, "DateDay").ToString()) %>:<%# CheckLen(DataBinder.Eval(Container.DataItem, "EntryHour").ToString()) %>', <%# DataBinder.Eval(Container.DataItem, "avgbatteryv") %>],
  </ItemTemplate>
</asp:Repeater>
            ]);
