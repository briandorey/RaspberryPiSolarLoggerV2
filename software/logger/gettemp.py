...
def formatResult(resultvalue):
	if resultvalue is not None:
		if resultvalue.startswith("TBTEMP"):
			...
			print("TB Living Room: %.1f" % round(float(valnum),2))
			
		if resultvalue.startswith("TATEMP"):
			valnum = resultvalue.replace("TATEMP", "")
			print("TA Andrew Bedroom: %.1f" % round(float(valnum),2))
		if resultvalue.startswith("TCTEMP"):
			valnum = resultvalue.replace("TCTEMP", "")
			print("TC Brian Bedroom: %.1f" % round(float(valnum),2))
